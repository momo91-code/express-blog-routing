...
router.get('/:id', (req, res) => {
    res.send(`You requested the post with id: ${req.params.id}`)
});

// questo codice mostra il post che hai chiesto di vedere con id
/*router.get('/:id', (req, res) => {
...
router.delete('/:id', (req, res) => {
    res.send(`You requested to destroy the post with id: ${req.params.id}`)
});

// questo codice mostra il codice trovato da eliminare
/*router.delete('/:id', (req, res) => {
    const id = parseInt(req.params.id)
    const index = posts.findIndex(p => p.id === id)
    if (index !== -1) {
